...
def search_concept(n_clicks, search_term, search_graph):
    ...
    if n_clicks:  # or triggered_id == "search-term") and not disabled:
        ...
        if search_graph == "wikidata":
            concepts = backend_api.get_json(
                f"/wikidata/search", retries=5, term=search_term.strip())
        else:
            concepts = backend_api.get_json(
                f"/search", retries=5, term=search_term.strip())
        # print(concepts)
        ...
        if concepts:
            ...
            table = dash_table.DataTable(
                ...
                style_table={'overflowX': 'auto',
                             'overflowY': 'auto', 'max-height': '60vh'},
